    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
      <ul class="navbar-nav">
        ...
        <li class="nav-item active">	    
          <a class="nav-link" href="/podat-obyavlenie?lang={{$language}}">Подать объявлениe <span class="sr-only">(current)</span></a>
        </li>	
        <li class="nav-item">
  ...
  <div class="col-sm-12 col-md-12 col-lg-12 col-xl-6 center">
    ...
    <form @submit="search">
      ...
      <div class="index_select_region_and_other_button_block">    
        <button class="btn btn-link" data-toggle="modal" id="locationButton" style="margin-top:-8px" @click="showLocationWindow"><b>расположение {{ $locationName }}</b></button>
        </div>
      </div>

    <div class="col-sm-12 col-md-12 col-lg-3 col-xl-3 center" title="Подать новое объявление" id="new_advert_col">
      <a href="/podat-obyavlenie?lang={{$language}}"><div id="new_advert_block">подать объявление</div></a>    
    </div>
    ...
    <div class="center">        
      <div id="categories_title" class="shadow_text" style="margin-bottom:80px"></div>    
          ...
          @foreach($categories as $category)
              ...
              @if ($location==="/")
                <a href="/category/{{ $category['url'] }}/?lang={{$language}}" class="url"><div class="category_item">{{ $category["name"] }}</div></a>
              @else
                <a href="/{{ $location }}/category/{{ $category['url'] }}/?lang={{$language}}" class="url"><div class="category_item">{{ $category["name"] }}</div></a>
              @endif
        	  </div>
          @endforeach
	      </div>
         ...
         <button type="button" style="font-size:15px;color:rgb(80,80,80);font-weight:bold" id="close_subcats_btn" class="btn btn-link hide" @click="returnToCategories">&#8634; назад</button>         
          <div id="subcategories" class="form-inline center">
            @foreach($subcategories as $subcategory)
              ...
              @if ($location==="/")
                <a href="/category/{{ $subcategory['category_url']}}/{{ $subcategory['url'] }}/?lang={{$language}}" class="url"><div class="category_item subcategory">{{ $subcategory["name"] }}</div></a>
              @else
                <a href="/{{ $location }}/category/{{ $subcategory['category_url'] }}/{{ $subcategory['url'] }}/?lang={{$language}}" class="url"><div class="category_item subcategory">{{ $subcategory["name"] }}</div></a>
              @endif
              </div>
            @endforeach
          </div>

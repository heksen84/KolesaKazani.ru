<template>
  <b-container fluid>
    ...
    <b-row style="margin-top:2px">
        ...
        <a href="/">
          <div id="logo_block">
            <div style="font-size:178%;letter-spacing:4px;font-weight:450;padding:7px;margin-top:-8px">АксуМаркет<span style="position:relative;top:-15px;left:-3px;font-size:12px;display:none">&reg</span>
            </div>
    ...
    <b-row v-for="i in Math.ceil(Object.keys(items).length / 4)" v-bind:key=i>
      <b-col cols="12" sm="12" md="12" lg="3" xl="3" v-for="item in items.slice((i - 1) * 4, i * 4)"  v-bind:key=item.id>
        ...
        <div class="category_item">{{ item.name }}<span style="font-size:13px;color:rgb(160,255,160);float:right;margin-top:4px" :id="item.id">| {{ 555 }} </span></div></a>
      </b-col>
    </b-row>
